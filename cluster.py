    ...
    while boundingboxes.shape[0]:
        ...
        for idx, n in enumerate(nearest_elements[0]):
            if idx != 0:
                ...
                expected_distance = ref_rect[2] / 2 + neighb_rect[
                    2] / 2  # half width of reference + half width of candidate
        ...
        if (len(candidate_neighbours) != 0):
            c = sorted(candidate_neighbours,
                       key=lambda e: np.pi * 2 - abs(e[3]))  # choose the most horizontal nearest neighbour
